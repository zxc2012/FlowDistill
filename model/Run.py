...
from lib.TrainInits import get_memory_usage, init_seed
...
def scaler_mae_loss(mask_value):
    def loss(preds, labels, scaler, mask=None, alpha =1.0):
...
def scaler_huber_loss(mask_value):
    def loss(preds, labels, scaler, mask=None, alpha = 1.0):
        ...
        if args.mode == 'pretrain' and mask is not None:
            preds = preds * mask
            labels = labels * mask
        mae = huber_loss(preds, labels, mask_value, alpha)
        return mae
    return loss

if args.loss_func == 'mask_mae':
    loss = scaler_mae_loss(mask_value=args.thresh)
    print('============================scaler_mae_loss')
elif args.loss_func == 'mask_huber':
    if args.mode != 'pretrain':
        loss = scaler_huber_loss(mask_value=args.thresh)

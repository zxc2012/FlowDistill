...
class OutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # (batch_size, output_dim, T, num_nodes)
...
class STGCN(nn.Module):
    def __init__(self, args_predictor, device, dim_in, dim_out):
        ...
        self.device = device
        self.IB_size = args_predictor.IB_size
    ...
    def forward(self, x, select_dataset):
        # print(x.shape) # (Batch_size, input_length, num_nodes, 1)
        ...
        # print(x_st2.shape)
        mu = x_st2[:,:self.IB_size, :,:]
        std = F.softplus(x_st2[:, self.IB_size:,:,:], beta=1)
        x_st2 = self.reparametrize_n(mu, std, self.num_sample)
        if self.num_sample == 1 : pass
        elif self.num_sample > 1 : x_st2 = x_st2.mean(0)

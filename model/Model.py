...
class Traffic_model(nn.Module):
    def __init__(self, args, args_predictor):
        ...
        elif self.model == 'KDMLP':
            from KDMLP.KDMLP import KDMLP
            self.predictor = KDMLP(args_predictor)
        else:
            raise ValueError

    def forward(self, source, label, select_dataset, batch_seen=None):
        if self.model == 'OpenCity' or self.model == 'KDMLP':
            x_predic = self.predictor(source, select_dataset)
        else:

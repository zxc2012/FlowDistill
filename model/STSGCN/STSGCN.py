...
class OutputLayer(nn.Module):
    def __init__(self, num_of_vertices, input_length, num_of_features,
        ...
        self.num_of_vertices = num_of_vertices
        self.input_length = input_length
        self.num_of_features = int(num_of_features/2)
...
class STSGCN(nn.Module):
    def __init__(self, args_predcitor, device, dim_in, dim_out):
        ...
        self.IB_size = args_predcitor.IB_size
        self.num_sample = args_predcitor.num_sample
    ...
    def forward(self, source, select_dataset):
        ...
        mu = data[:,:,:,:self.IB_size]
        std = F.softplus(data[:, :,:, self.IB_size:], beta=1)
        data = self.reparametrize_n(mu, std, self.num_sample)
        if self.num_sample == 1 : pass
        elif self.num_sample > 1 : data = data.mean(0)

...
def time_add(data, week_start, interval=5, weekday_only=False, holiday_list=None, day_start=0, hour_of_day=24):
    ...
    for index in range(day_start//interval, data.shape[0]+day_start//interval):
        if (index) % time_slot == 0 and index!=0:
            day_init = 0
        day_init = day_init + interval/interval
        if (index) % time_slot == 0 and index !=0:
        ...
        day_data[index:index + 1, :] = day_init - 1
        week_data[index:index + 1, :] = week_init - 1
...
# load dataset
def load_st_dataset(dataset, args, tea= False):
    # =========== Traffic flow (PEMS) =========== #
    ...
    # 1 / 1 / 2016 - 12 / 31 / 2021 Friday
        ...
        data_path = os.path.join(f'../data/{dataset}/{dataset}.npz')
        if tea == True:
            data_path = os.path.join(f'../data/{dataset}/NYCtea_TAXI.npz')
        if args.mode == 'pretrain':
            # 2016-2020 data
            data = np.load(data_path)['data'][:, :int((366+365+365+365+366)*48), 0].transpose(1, 0)
        else:
            data = np.load(data_path)['data'][:, :int((366)*48), 0].transpose(1, 0)
    ...
    # 1 / 1 / 2021 - 12 / 31 / 2021 Friday
        ...
        if tea == True:
            data_path = os.path.join(f'../data/{dataset}/CHItea_TAXI.npz')
        else:
            data_path = os.path.join(f'../data/{dataset}/{dataset}.npz')
        data = np.load(data_path)['data'][:, :, 1].transpose(1, 0)
        data[np.isnan(data)] = 0
        week_start = 5
        interval = 30
        week_day = 7
...
def split_data_by_ratio(data, val_ratio, test_ratio, train_ratio):
    ...
    if test_ratio == 0:
        ...
        val_data = data[-int(data_len*(test_ratio+val_ratio)):-int(data_len*test_ratio)]
        train_data = data[:int(data_len*(train_ratio))]
...
class TrafficDataset(Dataset):
    def __init__(self, data, data_tea, batch_size, input_window=288, output_window=288, eval_only=False):
        self.data = data
        ...
        self.windows_tea = [
            (data_tea[i:i + input_window], data_tea[i + input_window:i + input_window + output_window])
            for i in range(len(data_tea) - input_window - output_window + 1)
        ]
        ...
        if eval_only==False:
            ...
            if len(self.windows) % batch_size != 0:
                ...
                self.windows_tea = self.windows_tea[:-(len(self.windows_tea) % batch_size)]
        ...
        self.batches_tea = [
            self.windows_tea[i:i + batch_size]
            for i in range(0, len(self.windows_tea), batch_size)
        ]
    ...
    def __getitem__(self, idx):
        ...
        batch_x_tea, batch_y_tea = zip(*self.batches_tea[idx])
        return torch.from_numpy(np.stack(batch_x)).float(), [torch.from_numpy(np.stack(batch_y)).float(), torch.from_numpy(np.stack(batch_y_tea)).float()]
...
def define_dataloder(args):
    ...
    #     # dataloder_val_list.append(datasets_val)  File "/mnt/RAID/opencity/model/BasicTrainer.py", line 170, in test
    ...
    for dataset_name in args.dataset_use:
        print(args.dataset_use, dataset_name, args.val_ratio, args.test_ratio)
        # print(sss)
        data = load_st_dataset(dataset_name, args, tea = False)
        data_tea = load_st_dataset(dataset_name, args, tea = True)
        num_nodes_dict[dataset_name] = data.shape[1]
        data_train, data_val, data_test = split_data_by_ratio(data, args.val_ratio, args.test_ratio, args.train_ratio)
        datatea_train, datatea_val, datatea_test = split_data_by_ratio(data_tea, args.val_ratio, args.test_ratio, args.train_ratio)
        print('data_train', data_train.shape, data_val.shape, data_test.shape)
        if args.real_value == False:
            scaler_data, scaler_day, scaler_week = normalize_dataset(data_train, args.input_base_dim)
            data_train[..., :args.input_base_dim] = scaler_data.transform(data_train[:, :, :args.input_base_dim])
            ...
            datatea_train[..., :args.input_base_dim] = scaler_data.transform(datatea_train[:, :, :args.input_base_dim])
            datatea_val[..., :args.input_base_dim] = scaler_data.transform(datatea_val[:, :, :args.input_base_dim])
            datatea_test[..., :args.input_base_dim] = scaler_data.transform(datatea_test[:, :, :args.input_base_dim])
        ...
        iw = args.his // (intervel // 5)
        ow = args.pred // (intervel // 5)
        datasets_train = TrafficDataset(data_train, datatea_train, args.batch_size, input_window=iw, output_window=ow, eval_only=False)
        datasets_val = TrafficDataset(data_val, datatea_val, args.batch_size, input_window=iw, output_window=ow, eval_only=True)
        datasets_test = TrafficDataset(data_test, datatea_test, args.batch_size, input_window=iw, output_window=ow, eval_only=True)
        dataloder_trn_list.append(datasets_train)
